$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '676d05478a8a7c87622289b2b3d68b99';
    var title = $('#autoComplete').val();   // Changed from $('.movie').val()
    if (title.trim() === "") {
...
function get_movie_cast(movie_id,my_api_key){
  ...
  $.ajax({
    ...
    success: function(my_movie){
      var limit = Math.min(my_movie.cast.length, 10);
    ...
    $.ajax({
      ...
      success: function(m_data){
        if(m_data.results.length > 0){
          ...
          arr_poster_list.push(''); // fallback empty string if no poster found
        }
      },
      error: function(){
        alert("Invalid Request!");
